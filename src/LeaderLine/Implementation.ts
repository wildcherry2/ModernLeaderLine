...
import { DefaultDashAnimationConfig, DefaultDashConfig, DefaultStyleConfig, ESocket, ThreeHundredSixtyDegrees } from "./Constants";
import type { CornerAngles, LeaderLineDashAnimationConfig, LeaderLineDashConfig, LeaderLineEventMap, LeaderLineReference, LeaderLineStyleConfiguration, Point2D } from "./Types";
...
class LeaderLineImplementation {
     ...
     * to the SVG elements.
     */
    constructor(public host: LeaderLine) {
     ...
     */
    get source(): LeaderLineReference { return this._source; }
    ...
    set source(src: Element) {
        src ||= null;
        if(src === this._source) return;
        if(src === this._target) throw new TypeError("leader-line source can't be the same as its target!");
     ...
     */
    get target(): LeaderLineReference { return this._target; }
    ...
    set target(trg: Element) {
        trg ||= null;
        if(trg === this._target) return;
        if(trg === this._source) throw new TypeError("leader-line target can't be the same as its source!");
     ...
     */
    get referencesSet(): boolean { return !!(this._source && this._target) }
     ...
     */
    async position() {
        if(this._positioning) return;
        this._positioning = true;
        if(!this.referencesSet) return this.hide(true);
        ...
        if(src_rect.intersectsRect(dest_rect)) return this.hide(true);
        let src_center: Point2D, dest_center: Point2D
     ...
     */
    hide(unset_positioning?: boolean) {
        if(unset_positioning) this._positioning = false;
        if(this._hidden) return;
     ...
     */
    drawLine(src: Point2D, dest: Point2D) {
        ...
        bounding_rect.x -= offset; bounding_rect.y -= offset; bounding_rect.width += TwoOffset; bounding_rect.height += TwoOffset; // need to account for text height?
        if(!dispatchEvent(this.host, 'leader-line-draw', {start: src, end: dest, rect: bounding_rect})) return;
     ...
     */
    getPathCommand(start: Point2D, end: Point2D) {
        if(this._source && this._source.leader_line_socket !== undefined) {
            if(this._source.leader_line_socket === ESocket.left) start.x += (this._style.line_thickness / 2);
            else if(this._source.leader_line_socket === ESocket.top) start.y += (this._style.line_thickness / 2);
            else if(this._source.leader_line_socket === ESocket.right) start.x -= (this._style.line_thickness / 2);
            else start.y -= (this._style.line_thickness / 2);
        }
     ...
     */
    getStyleConfig(): Readonly<LeaderLineStyleConfiguration> { return this._style; }
     ...
     */
    setStyleConfig(config: Partial<Readonly<LeaderLineStyleConfiguration>>) {
     ...
     */
    static getAngle(origin: Point2D, vert: Point2D) {
        let angle = Math.atan2(-(vert.y - origin.y), vert.x - origin.x);
        if(angle < 0) angle += ThreeHundredSixtyDegrees;
        return angle;
    }
     ...
     * @returns 
     */
    static getCornerAngles(rect: DOMRect, center = rect.getCenter()): CornerAngles {
     ...
     */
    static getSocketFromCornerAngle(incident: number, corners: CornerAngles) {
        if(incident <= corners.top_right || incident > corners.bottom_right) return ESocket.right;
        if(incident <= corners.top_left && incident > corners.top_right) return ESocket.top;
        if(incident <= corners.bottom_left && incident > corners.top_left) return ESocket.left;
     ...
     */
    static getSocketCoordsFromRect(rect: DOMRect, socket: ESocket, center: Point2D = rect.getCenter()) {
        if(socket === ESocket.left) center.x -= (rect.width / 2);
        else if(socket === ESocket.top) center.y -= (rect.height / 2);
        else if(socket === ESocket.right) center.x += (rect.width / 2);
        else center.y += (rect.height / 2);
        return center;
    }

    private _source: LeaderLineReference = null;
    private _target: LeaderLineReference = null;
    private _hidden = true;
    private _positioning = false;
    private _style = DefaultStyleConfig;
    private _anim: SVGAnimateElement;
    ...
    private applyStyleConfig() {
        ...
        if(this._style.dashed) { // if this was set to true in passed in config, it would be resolved to a default object and never be a boolean, so the boolean case doesn't have to be handled.
            const dash_config = this._style.dashed as LeaderLineDashConfig;
            ...
            if(dash_config.animate) { // if this was set to true in passed in config, it would be resolved to a default object and never be a boolean, so the boolean case doesn't have to be handled.
                const anim_config = dash_config.animate as LeaderLineDashAnimationConfig;
                if(!this._anim) {
        ...
        if(this._style.text?.length) {
            ...
            tpath ??= this._text.firstElementChild as SVGTextPathElement;
     ...
     */
    private assignNewReference(ref: Element, type: 'source' | 'target'): asserts ref is LeaderLineReference { // TODO replace source | target with boolean param
        const _ty = '_' + type as `_${typeof type}`;
        const old = this[_ty];
        ref && makeFastBoundsElement(ref);
        if(!dispatchEvent(this.host, 'leader-line-ref-change', {addedRef: ref as any, referenceType: type, removedRef: old})) return this.hide();
        this[_ty] = ref as any;
     ...
     */
    private static resolveStyleConfig(config: Required<LeaderLineStyleConfiguration>) {
        if(config.dashed) {
            if(typeof config.dashed === 'boolean') config.dashed = DefaultDashConfig;
            else {
                config.dashed = {...DefaultDashConfig, ...config.dashed};
                if(config.dashed.animate) {
                    if(typeof config.dashed.animate === 'boolean') config.dashed.animate = DefaultDashAnimationConfig;
                    else config.dashed.animate = { ...DefaultDashAnimationConfig, ...config.dashed.animate }
                }
            }
        }
    }
 ...
 * @returns True if preventDefault was not called, false otherwise.
 */
function dispatchEvent<K extends keyof LeaderLineEventMap>(host: LeaderLine, type: K, detail: Omit<LeaderLineEventMap[K]['detail'], 'line'>) {
    detail['line'] = host;
 ...
 */
export function makeLeaderLineImplementation(host: LeaderLine) {
    host.features ??= {} as any;
